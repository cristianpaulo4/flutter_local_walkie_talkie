...
class DiscoveryManager {
  ...
  final _devicesController =
      StreamController<List<WalkieTalkieDevice>>.broadcast();
  final Set<WalkieTalkieDevice> _foundDevices = {};

  BonsoirDiscovery? _discovery;
  BonsoirBroadcast? _broadcast;

  Stream<List<WalkieTalkieDevice>> get devicesStream =>
  ...
  Future<void> startDiscovery() async {
    ...
    _discovery!.eventStream!.listen((event) {
      if (event is BonsoirDiscoveryServiceFoundEvent) {
        ...
        String ipAddress = (ipFromAttr != null && ipFromAttr.isNotEmpty)
            ? ipFromAttr
            : (service.host ?? '');
        ...
        if (ipAddress.isEmpty) {
          debugPrint('Warning: Resolved service ${device.name} but IP is empty.');
        } else {
          debugPrint(
            'Successfully resolved: ${device.name} at ${device.ip}:${device.port} (Source: ${ipFromAttr != null ? 'Attribute' : 'Host'})',
          );
        }
        ...
        debugPrint('Service lost: $serviceName');
  ...
  Future<void> registerService(String name, int port, {String? ip}) async {
    final Map<String, String> attributes = {'info': 'WalkieTalkie Device'};
    if (ip != null) {
      attributes['ip'] = ip;
    }
    ...
    _broadcast!.eventStream!.listen((event) {
      debugPrint('Broadcast event: $event');
    });
